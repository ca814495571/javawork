...
import com.cqfc.common.zookeeper.DBKeyValueCache;
 ...
 *
 */
public class DBKeyValueWatcher implements WatcherOpeartion,Watcher {
		...
		if(zooKeeper != null){
			try {
				...
				for(String nodeName:childList){
					...
					String dbConfigValue = new String(nodeData);
					dbConfigMap.put(nodeName, dbConfigValue);
				}
				...
				DBKeyValueCache.setKeyValueMap(dbConfigMap);
			} catch (Exception e) {
				Log.run.error(e.getMessage(), e);
			}
		}
