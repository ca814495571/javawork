...
public interface StationInfoMapper extends BaseMapper {
	...
	@Select("SELECT * FROM t_station_info where id= #{id}")
	public StationInfo getStationInfoById(@Param("id") int id);

	@Select("select *  from  t_station_info ")
	...
	@Update("update t_station_info set "
			...
			+ "stationCreateTime=#{stationCreateTime},stationFlag=#{stationFlag},lastTime=#{lastTime},parentStationName=#{parentStationName} "
			+ "enterTag = #{enterTag},active = #{active} where id = #{id}")
	public int updateStationInfo(StationInfo stationInfo);
	
	
	@Select("select *  from t_station_info where ${where} and active = 1")
