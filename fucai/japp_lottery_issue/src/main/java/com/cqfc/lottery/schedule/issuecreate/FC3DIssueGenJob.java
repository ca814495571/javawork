...
/**
 * 福彩3D期号创建：如果能创建期号的开始时间 - 当前时间 < 6,则一次创建7天的期号
 * 
 * @author liwh
 */
@EnableScheduling
@Service
public class FC3DIssueGenJob {
	...
	public void execute() {
		...
		ScanLog.scan.info("-----------福彩3D期号创建定时任务开始------------");
		try {
			LotteryIssue lotteryIssue = lotteryIssueService.getMaxLotteryIssue(IssueConstant.LOTTERYID_FC3D);
			if (null != lotteryIssue) {
				...
				if (offset < createDays) {
					...
					lotteryIssueService.createLotteryIssue(createIssueBeginTime, endTime, IssueConstant.LOTTERYID_FC3D);
					ScanLog.scan.info("D3 create,beginTime=%s,endTime=%s", createIssueBeginTime, endTime);
				}
			}
		} catch (Exception e) {
			ScanLog.scan.error("七乐彩期号创建定时任务发生异常", e);
		}
		ScanLog.scan.info("-----------福彩3D期号创建定时任务结束------------");
	}
