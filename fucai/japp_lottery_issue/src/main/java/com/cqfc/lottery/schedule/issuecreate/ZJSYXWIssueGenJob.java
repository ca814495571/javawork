...
/**
 * 浙江11选5期号创建：如果能创建期号的开始时间 - 当前时间 < 6,则一次创建7天的期号
 * 
 * @author liwh
 */
@EnableScheduling
@Service
public class ZJSYXWIssueGenJob {
	...
	public void execute() {
		...
		ScanLog.scan.info("-------------浙江11选5期号创建定时任务开始-------------");
		try {
			LotteryIssue lotteryIssue = lotteryIssueService.getMaxLotteryIssue(IssueConstant.LOTTERYID_ZJSYXW);
			if (null != lotteryIssue) {
				...
				if (offset < createDays) {
					...
					lotteryIssueService.createLotteryIssue(createIssueBeginTime, endTime,
							IssueConstant.LOTTERYID_ZJSYXW);
					ScanLog.scan.info("ZJSYXW create,beginTime=%s,endTime=%s", createIssueBeginTime, endTime);
				}
			}
		} catch (Exception e) {
			ScanLog.scan.error("浙江11选5期号创建定时任务发生异常", e);
		}
		ScanLog.scan.info("-------------浙江11选5期号创建定时任务结束--------------");
	}
