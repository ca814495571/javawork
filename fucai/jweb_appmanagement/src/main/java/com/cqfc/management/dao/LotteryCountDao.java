...
public class LotteryCountDao {
	...
	public void filterNullColumn(LotteryCount LotteryCount) {
		...
		if (LotteryCount.getEnterTag() == null) {

			LotteryCount.setEnterTag("");
		}

	}

	public List<LotteryCount> getLotteryCountByDate(StationInfo stationInfo,
			String fromDate, String toDate) {

		return lotteryCountMapper.getLotteryCountByTime(stationInfo, fromDate,
				toDate);
	}
	...
	public List<LotteryCount> getLotteryCountByDate(StationInfo stationInfo,
		...
		return lotteryCountMapper.getLotteryCountByDate(stationInfo, fromDate,
				toDate, pageNum * (pageSize - 1), pageSize);
	}

	public int getRecordTotal(StationInfo stationInfo, String fromDate,
			String toDate) {

		return lotteryCountMapper.getRecordTotal(stationInfo, fromDate, toDate);
	}

	public int getTotalSaleNum(StationInfo stationInfo, String lotteryType,
			String fromDate, String toDate) {

		return lotteryCountMapper.getTotalSaleNum(stationInfo, lotteryType,
				fromDate, toDate);
	}

	public int deleteAll(String date){
		return lotteryCountMapper.deleteAll(date);
	}
}
