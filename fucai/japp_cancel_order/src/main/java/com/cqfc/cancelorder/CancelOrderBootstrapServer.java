...
import com.cqfc.protocol.cancelorder.CancelOrderService.Iface;
import com.cqfc.util.ParameterUtils;

@Component
public class CancelOrderBootstrapServer {

	@Resource
	CancelOrderService.Iface cancelOrderHandler;
	...
	public static void main(String[] args) {
		...
		model.setIdlList(ServerUtil.getIdlList(CancelOrderService.Iface.class));
		...
		CancelOrderBootstrapServer server = (CancelOrderBootstrapServer) applicationContext
				.getBean("cancelOrderBootstrapServer");
		server.start();
	}

	private void start() {
		System.out.println("starting cancelOrderBootstrapServer ...");
		CancelOrderService.Processor<Iface> cancelOrder = new CancelOrderService.Processor<Iface>(
				this.cancelOrderHandler);
		try {
			int port = Integer.valueOf(ParameterUtils.getParameterValue("SERVER_PORT"));
			TServerTransport transport = new TServerSocket(port);
			TThreadPoolServer.Args serverArgs = new TThreadPoolServer.Args(transport);
			serverArgs.processor(cancelOrder);
