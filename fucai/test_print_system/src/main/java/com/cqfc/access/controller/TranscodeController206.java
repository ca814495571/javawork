...
import com.cqfc.access.model.Model206;
...
public class TranscodeController206 {
	@RequestMapping(value="/transcode206")
	public String savePersonUI(){				
		return "transcode206";
	}
	
	@RequestMapping(value="/request206", produces = { "text/xml;charset=UTF-8" })
	@ResponseBody
	public String save(Model206 model206) throws UnknownHostException, UnsupportedEncodingException{		
		...
		String sign = MD5Utils.getMD5(channelID + model206.getLotteryID() + model206.getWareIssue() + key);		
		String result = "";
		...
		try{
			...
			// 绑定到请求 Entry		           
			formParams.add(new BasicNameValuePair("transcode", ConstantsUtil.TRANSCODE206));
			formParams.add(new BasicNameValuePair("ChannelID", channelID));
			formParams.add(new BasicNameValuePair("LotteryID", model206.getLotteryID()));
			formParams.add(new BasicNameValuePair("WareIssue", model206.getWareIssue()));
