...
import com.cqfc.access.model.Model302;
...
public class TranscodeController302 {
	@RequestMapping(value="/transcode302")
	public String savePersonUI(){				
		return "transcode302";
	}
	
	@RequestMapping(value="/request302", produces = { "text/xml;charset=UTF-8" })
	@ResponseBody
	public String save(Model302 model302) throws UnknownHostException, UnsupportedEncodingException{		
		...
		String sign = MD5Utils.getMD5(channelID + model302.getLotteryID() + model302.getWareIssue() + key);		
		String result = "";
		...
		try{
			...
			// 绑定到请求 Entry		           
			formParams.add(new BasicNameValuePair("transcode", ConstantsUtil.TRANSCODE302));
			formParams.add(new BasicNameValuePair("ChannelID", channelID));
			formParams.add(new BasicNameValuePair("LotteryID", model302.getLotteryID()));
			formParams.add(new BasicNameValuePair("WareIssue", model302.getWareIssue()));
