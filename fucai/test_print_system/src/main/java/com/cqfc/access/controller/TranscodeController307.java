...
import com.cqfc.access.model.Model307;
...
public class TranscodeController307 {
	@RequestMapping(value="/transcode307")
	public String savePersonUI(){				
		return "transcode307";
	}
	
	@RequestMapping(value="/request307", produces = { "text/xml;charset=UTF-8" })
	@ResponseBody
	public String save(Model307 model307) throws UnknownHostException, UnsupportedEncodingException{		
		...
		String sign = MD5Utils.getMD5(channelID + model307.getChannelTicketID() + key);		
		String result = "";
		...
		try{
			...
			// 绑定到请求 Entry		           
			formParams.add(new BasicNameValuePair("transcode", ConstantsUtil.TRANSCODE307));
			formParams.add(new BasicNameValuePair("ChannelID", channelID));
			formParams.add(new BasicNameValuePair("ChannelTicketID", model307.getChannelTicketID()));
