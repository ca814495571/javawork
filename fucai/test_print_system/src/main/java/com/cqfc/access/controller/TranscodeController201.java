...
import com.cqfc.access.model.Model201;
...
public class TranscodeController201 {
	@RequestMapping(value="/transcode201")
	public String savePersonUI(){				
		return "transcode201";
	}
	
	@RequestMapping(value="/request201", produces = { "text/xml;charset=UTF-8" })
	@ResponseBody
	public String save(Model201 model201) throws UnknownHostException, UnsupportedEncodingException{		
		...
		String sign = MD5Utils.getMD5(channelID + model201.getLotteryID() + key);
		String result = "";
		...
		try{
			...
			// 绑定到请求 Entry		           
			formParams.add(new BasicNameValuePair("transcode", ConstantsUtil.TRANSCODE201));
			formParams.add(new BasicNameValuePair("ChannelID", channelID));
			formParams.add(new BasicNameValuePair("LotteryID", model201.getLotteryID()));
