...
import com.cqfc.access.model.Model202;
...
public class TranscodeController202 {
	@RequestMapping(value="/transcode202")
	public String savePersonUI(){				
		return "transcode202";
	}
	
	@RequestMapping(value="/request202", produces = { "text/xml;charset=UTF-8" })
	@ResponseBody
	public String save(Model202 model202) throws UnknownHostException, UnsupportedEncodingException{		
		...
		String sign = MD5Utils.getMD5(channelID + model202.getLotteryID() + key);
		String result = "";
		...
		try{
			...
			// 绑定到请求 Entry		           
			formParams.add(new BasicNameValuePair("transcode", ConstantsUtil.TRANSCODE202));
			formParams.add(new BasicNameValuePair("ChannelID", channelID));
			formParams.add(new BasicNameValuePair("LotteryID", model202.getLotteryID()));
			formParams.add(new BasicNameValuePair("MatchType", model202.getMatchType()));
