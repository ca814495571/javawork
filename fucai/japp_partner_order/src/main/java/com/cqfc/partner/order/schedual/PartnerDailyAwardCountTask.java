...
@EnableScheduling
@Service
public class PartnerDailyAwardCountTask {
	...
	public void executeDailySaleCount() {
		...
		Log.run.info("时间--" + countTime + "统计面向用户的所有合作商兑奖总额start..");
		int flag = -1;
		int i=1;
		while(i < 4 && flag!=1){
		
			flag = partnerOrderService.partnerDailyAwardCount(countTime);
			if(flag != 1){
				Log.run.warn("时间" + countTime + "日兑奖金额统计失败"+i+"次");
				if(i == 3){
					Log.error("时间" + countTime + "日兑奖金额统计失败"+i+"次");
				}
				try {
					Thread.sleep(60000);
				...
				Log.run.info("时间" + countTime + "日兑奖金额统计成功!!");
				break;
			}
			i++;
		}
