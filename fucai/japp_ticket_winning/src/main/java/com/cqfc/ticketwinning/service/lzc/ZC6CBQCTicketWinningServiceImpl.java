...
public class ZC6CBQCTicketWinningServiceImpl implements ITicketWinningService{
	/**
	 * 校验投注内容
	 * @param orderContent 11-00-11-00-01-31
	 * @return
	 */
	public boolean validateContent(String orderContent, String playType) {
		boolean flag = true;		
		Set<String> contentSet = LZCContentSetUtil.getSetContentByLotteryId(LotteryType.ZC6CBQC.getText());
		
		if(playType.equals(LotteryPlayTypeConstants.PLAYTYPE_LZC_6_DANSHI)){
		...
		else if(playType.equals(LotteryPlayTypeConstants.PLAYTYPE_LZC_6_FUSHI)){
	...
	public int calBallCounts(String orderContent, String playType) {
		...
		for(int i = 0; i< ballContents.length; i++){
			sum += calZC6CBQCBallCount(ballContents[i], playType);
		}
		
		return sum;
	}
	
	private int calZC6CBQCBallCount(String orderContent, String playType){
		int count = 0;
	...
	public List<String> listTicketDetails(String orderContent, String playType) {
		...
		for(int i = 0; i < ballContents.length; i++){
			details.addAll(listZC6CBQCTicketDetail(ballContents[i], playType));
		}
		
		return details;
	}
	
	private List<String> listZC6CBQCTicketDetail(String orderContent, String playType){
		List<String> details = new ArrayList<String>();
	...
	public Prize calTicketWinningAmount(String orderContent, String playType, String winningContent, Map<Integer, Long> prizeAmountLevelMap){
		...
		if(playType.equals(LotteryPlayTypeConstants.PLAYTYPE_LZC_6_DANSHI)){
			String[] ballContents = orderContent.split(TicketWinningConstantsUtil.SEPARATOR_JINGHAO);
			for(int i = 0, len = ballContents.length; i < len; i++){
				prizeLevel = calBQCWinningTicketPrize(ballContents[i], winningContent);	
			...
			for(String ballContent : ballContents){
				prizeLevel = calBQCWinningTicketPrize(ballContent, winningContent);	
				prizeAmount = prizeAmountLevelMap.containsKey(prizeLevel) ? prizeAmountLevelMap.get(prizeLevel) : 0;
				if (prizeLevel > 0) {
					if(prizeAmount == 0){
						prize.setPrize(true);
	...
	private int calBQCWinningTicketPrize(String ballContent, String winningContent) {
		int prizeLevel = 0;
		...
		if(winningCount == 6){
			prizeLevel = PrizeAmount.ZC_6CBQC_FIRST_PRIZE_LEVEL;
		}
