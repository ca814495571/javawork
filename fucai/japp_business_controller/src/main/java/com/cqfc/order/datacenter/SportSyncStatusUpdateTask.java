 ...
 * @author liwh
 */
public class SportSyncStatusUpdateTask implements Runnable {

	private static final List<BlockingQueue<UpdateSyncStatus>> syncListQueue = SportOrderBuffer.getSyncListQueue();
	...
	public SportSyncStatusUpdateTask(ApplicationContext applicationContext, AtomicBoolean running, int index) {
	...
	public void run() {
		...
		try {
				...
				if (list.size() > 0) {
					try {
						...
						for (UpdateSyncStatus updateSyncStatus : list) {
							String tableName = DbGenerator.getSportOrderTableNameMain(updateSyncStatus.getTradeId());
							statement.addBatch(getUpdateSyncSql(updateSyncStatus, tableName));
							Log.fucaibiz.info("竞技彩,批量更新订单同步状态,orderNo=%d,dbName=%s,tableName=%s",
						...
						try {
							...
							for (int i = 0, len = list.size(); i < len; i++) {
								...
								Log.fucaibiz.info("竞技彩,批量更新订单同步状态成功,orderNo=%d,syncStatus=%d,returnValue=%d", orderNo,
										sync, returnValue);

								if (returnValue > 0) {
									OrderMemcacheUtil.updateSportOrderSyncMemcache(orderNo, sync);
								}
								updateSyncMap(orderNo, BatchConstant.BATCH_OPERATE_SUCCESS);
							}
						} catch (Exception e1) {
							Log.fucaibiz.error("竞技彩,批量更新订单同步状态异常,进行逐条更新操作", e1);
							long orderNo = 0;
							try {
								connection.rollback();
								...
								for (UpdateSyncStatus updateSyncStatus : list) {
									...
									int sync = updateSyncStatus.getSync();
									String tableName = DbGenerator.getSportOrderTableNameMain(updateSyncStatus
											.getTradeId());
									boolean operateValue = statement.execute(getUpdateSyncSql(updateSyncStatus,
											tableName));
									Log.fucaibiz.info("竞技彩,逐条更新订单同步状态完成,orderNo=%d,syncValue=%d,operateValue=%b",
											orderNo, sync, operateValue);
								...
								Log.fucaibiz.error("竞技彩,逐条更新订单同步状态发生异常", e2);
								// stop();
								// return;
							}
						}
					} catch (Exception e) {
						Log.fucaibiz.error("竞技彩,批量更新订单同步状态发生异常", e);
					} finally {
						list.clear();
						closeConnection(connection, statement);
						connection = null;
			...
			Log.fucaibiz.debug("竞技彩,batch update order sync error", e);
		}
	}
	...
	private String getUpdateSyncSql(UpdateSyncStatus updateSyncStatus, String tableName) {
		...
		try {
			...
			Log.fucaibiz.info("竞技彩,批更新订单同步状态拼接SQL,orderNo=%d,sql=%s", orderNo, sql);
		} catch (Exception e) {
			Log.fucaibiz.error("竞技彩,批更新订单同步状态拼接SQL发生异常,orderNo=" + orderNo, e);
		}
		return sql;
	}
	...
	private void updateSyncMap(long orderNo, int operateValue) {
		try {
			BatchConstant.sportOrderSyncMap.get(orderNo).setOperateIdentifier(operateValue);
			BatchConstant.sportOrderSyncMap.get(orderNo).setIsBackReq(true);
			UpdateSyncStatus syncMsg = BatchConstant.sportOrderSyncMap.get(orderNo);
			synchronized (syncMsg) {
				syncMsg.notify();
			}
		} catch (Exception e) {
			Log.fucaibiz.error("竞技彩,批更新订单同步状态后更新MAP发生异常,orderNo=" + orderNo, e);
		}
	}
	...
	private void closeConnection(Connection connection, Statement statement) {
		try {
			...
			Log.run.debug("竞技彩,batch operate order connection close error", e);
		}
	}
}
